...
class APHandler():
    def __init__(self, inst, sname, comm):
        ...
        self.dist_mat_input_file = inst.get(sname, "dist_mat_input_file")
        ...
        self.structure_dir = inst.get(sname, 'structure_dir')
        self.structrue_dir_depth = inst.get_with_default(sname, 'structrue_dir_depth' , 0, eval=True)
    ...
    def get_new_pref_range(self, num_of_clusters_and_pref_list, prev_l, prev_u, iter_n):
        ...
        pref_list.reverse()
        print('num_clusters_list', num_clusters_list, 'pref_list', pref_list)
        '''
        #Make it ascending order
        n = 1
        while n < len(num_clusters_list):
            if num_clusters_list[n] < num_clusters_list[n - 1]:
                num_clusters_list[n] = num_clusters_list[n - 1]
            n += 1
        ...
        for n, num_clusters in enumerate(num_clusters_list):
            if abs(num_clusters - self.num_of_clusters) < self.num_of_clusters_tolerance or\
                    (self.output_without_success and iter_n >= self.max_sampled_preferences):
                success = True
                
                if self.output_without_success and iter_n >= self.max_sampled_preferences:
                    n = len(num_clusters_list) -  num_clusters_list.index(min(num_clusters_list, key=lambda x:abs(x - self.num_of_clusters))) - 1
                else:
    ...
    def run_fixed_num_of_clusters(self):
        ...
        closest_result = None
        print('pref_l, pref_u' + str(pref_l) + ', ' + str(pref_u))
        ...
        while iter_n < self.max_sampled_preferences:
            ...
            print('self.preference', self.preference, 'rank', self.rank, 'num_of_clusters_gotten', num_of_clusters_gotten, 'iter_n', iter_n)
            ...
            if success:
                print('success')
                if n != self.rank:
                    print('rank ' + str(self.rank) + ' is returning')
                    return
