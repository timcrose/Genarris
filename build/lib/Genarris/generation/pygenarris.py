...
def pygenarris_structure_generation(inst=None, comm=None, filename=None, num_structures_per_allowed_SG_per_rank=None, Z=None, volume_mean=None, volume_std=None, sr=None, tol=None, max_attempts_per_spg_change=None, molecule_path=None, omp_num_threads=1):
    ...
    if inst is not None:
        ...
        max_attempts_per_spg_change = inst.get_eval(sname, 'max_attempts_per_spg_change')
    ...
    pygenarris.generate_molecular_crystals_with_vdw_cutoff_matrix(filename, cutoff_matrix, num_structures_per_allowed_SG_per_rank, Z, volume_mean, volume_std, tol, max_attempts_per_spg_change)
    if comm is not None:
        comm.barrier()
        if comm.rank == 0:
