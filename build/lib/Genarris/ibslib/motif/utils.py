...
def extract_molecules(struct, molecule_list, whole_molecules=True):
    ...
    if whole_molecules:
        molecule_struct_list = [construct_smallest_molecule(struct,x) 
                                 for x in molecule_struct_list]
    return molecule_struct_list


def construct_smallest_molecule(struct,molecule_struct):
    ...
    mol_index_list = find_molecules(molecule_struct)
    ...
    for i in range(2,5):
        ...
        result = find_molecules(supercell)
        # Get molecule structure
        molecule_list = extract_molecules(supercell, result, whole_molecules=False)
        # Identify whole molecule in non-periodic cell
        frag_list = [len(find_molecules(x)) for x in molecule_list]
        try:
