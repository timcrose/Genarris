...
def interatomic_distance_evaluation(inst):
	...
	if info_level>=2 or (inst.has_procedure("Interatomic_Distance_Evaluation")\
	and info_level>=1):
		write_log.write_master_log(inst,"interatomic_distance_evaluation has been called to evaluate %i structures" % len(struct_list))
	...
	if processes_limit > 1:
		...
		result = mulp.map(interatomic_distance_evaluation, inst_list)
		return result
	...
	for y in range (-1,2) for x in range (-1,2) if x!=0 or y!=0 or z!=0]
	...
	property_name = inst.get_with_default(sname,"property_name","interatomic_distance")

	for path in struct_list:
		original_struct = misc.load_structure_with_inst(inst,sname,"structure_format",path)
		struct = copy.deepcopy(original_struct)
		structure_handling.cell_modification(struct,struct.get_n_atoms()//napm,\
							napm, False)
	
		struct = structure_handling.cell_extension(struct,extension=\
				structure_extension, create_duplicate=False)
		result = []

		f = open("detailed_results.info","w") #!!!!!!
		for pair in atomic_pairs: 
		#Goes through all the pairs specified by user
			l = specific_interatomic_distances(struct,pair[0],pair[1],napm,\
		allow_all_pairs=allow_all_pairs, block_same_pair=block_same_pair)
		...
		if not output_file == False:
			...
			for r in result:
				message += " %i %i %f" % tuple(r)
			write_log.write_log(output_file,message,time_stamp=False)

		if inst.has_option(sname,"output_folder"):
			original_struct.properties[property_name] = result
...
#		if property_name in struct.properties:
#			continue  #!!!!!!!!!!!!!!!Temporary edits for restart!
...
def all_interatomic_distances (struct, a1, a2, a1_range=None, a2_range=None, napm=1):
	'''
...
def specific_interatomic_distances (struct, a1, a2, napm, allow_all_pairs=False, block_same_pair=False):
	'''
	Returns a list of interatomic distances
	...
	if isinstance(a1,int) and isinstance(a2,int):
		...
		if not block_same_pair:
			if not allow_all_pairs:
				if a2<napm:
				#Atoms must be fromt different molecules
				#Since allow_all_pairs set to False, a1 must be from the first molecule as well
					return []
				return all_interatomic_distances(struct, a1,\
				struct.geometry[a2]["element"], a1_range=range(0,napm), a2_range=[a2])
			else:
				return all_interatomic_distances(struct, a1,\
				struct.geometry[a2]["element"], a2_range = [a2],
				a1_range = range(0,napm*(a2//napm))+range(napm*(a2//napm+1),len(struct.geometry))) #Need to avoid the molecule of a2
		else:
			if not allow_all_pairs:
				if a2<napm:
					return []
				...
				for i in range (0,a2//napm)+range(a2//napm+1,nmpc):
					result.append(min_distance(all_interatomic_distances(\
					struct, a1, struct.geometry[a2]["element"],
