...
def estimate_unit_cell_volume(inst, comm):
    ...
    if comm.rank == 0:
        ...
        molecule_path = inst.get_inferred(sname, [sname, 'harris_single_molecule_prep', 'pygenarris_structure_generation', 'structure_generation_batch', 'harris_approximation_batch'], 
        ...
        vol_estimate = None
    std_about_experimental_vs_prediction_best_fit_line = 21.25
    num_std_devs_above_mean_for_99_confidence_interval = 2.5
    vol_estimate = comm.bcast(vol_estimate, root=0)
    vol_lower_bound = vol_estimate - 3.0 * std_about_experimental_vs_prediction_best_fit_line
    vol_upper_bound = vol_estimate + 3.0 * std_about_experimental_vs_prediction_best_fit_line
    vol_ratio_range = [vol_lower_bound / vol_estimate, vol_upper_bound / vol_estimate]
    if inst.has_section('pygenarris_structure_generation'):
        inst.set('pygenarris_structure_generation', 'volume_mean', str(vol_estimate))
        inst.set('pygenarris_structure_generation', 'volume_std', str(vol_std))
    if inst.has_section('structure_generation_batch'):
        inst.set('structure_generation_batch', 'ucv_target', str(vol_estimate))
