...
class ParallelCalc():
    def __init__(self, struct_path, output_path, ibslib_class, overwrite=False,
                 use_class_write=True, comm=None, verbose=True):
        """
    ...
    def _calc(self):
        file_list = os.listdir(self.struct_path)
        
        my_idx_start = (int(len(file_list) / self.size)+1)*self.rank
        my_idx_end = my_idx_start + int((len(file_list) / self.size) + 1)

        if my_idx_end > len(file_list):
            my_idx_end = len(file_list)
        
        all_idx = np.arange(my_idx_start, my_idx_end,1)
        total = all_idx.shape[0]
        for value in all_idx:
            file_name = file_list[value]
            file_path = os.path.join(self.struct_path, file_name)
            
            struct = read(file_path)
            self.ibslib_class.calc_struct(struct)
            
            temp_dict = {struct.struct_id: struct}
            write(self.output_path, temp_dict, overwrite=self.overwrite)
